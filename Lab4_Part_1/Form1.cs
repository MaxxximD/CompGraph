    ...
    public partial class Form1 : Form
    {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            int number_of_point = 0;
                ...
                if (comboBox_polygon.SelectedIndex == -1)
                {
                    MessageBox.Show("number_of_point fields", "Error", MessageBoxButtons.OK);
                    return;
                }
                number_of_polygon = comboBox_polygon.SelectedIndex;
                ...
                number_of_point = comboBox_point.SelectedIndex;
        
            Polygon polygon = polygonList[number_of_polygon];
            P p = pointList[number_of_point];
            bool check = PointInside(polygon, p);
            MessageBox.Show(check.ToString(), "done",MessageBoxButtons.OK);
        }

        private bool PointInside(Polygon polygon,P point) {
            Bitmap b = new Bitmap(pictureBox1.Height, pictureBox1.Width);
            Graphics g = Graphics.FromImage(b);
            Pen red = new Pen(Color.Red);
            polygon.DrawPolygon(g, red);
            int x = (int)point.x;           
            int y = (int)point.y;
            int xfound = 0;
            int count = 0;
            while (x < b.Width)
            {
                ...
                foreach (var po in polygon.points) {
                    if ((po.x == x&&po.y==y) || (po.x == (x + 1)&& po.y == (y + 1))|| (po.x == (x - 1)&& po.y == (y - 1))|| po.x == (x +1) && po.y ==y|| po.x == x && po.y == y)
                        containts = true;
                }
                if (b.GetPixel(x, y) == Color.FromArgb(255, 255, 0, 0) && (x - xfound > 1)&& !containts)
                 {
                    count++;
                    xfound = x;
                }
        ...
        // перечислимый тип для определения положения точки
        enum Position
        {
            Right, Left, Unknown 
        }
