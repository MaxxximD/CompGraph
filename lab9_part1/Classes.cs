...
namespace lab6
{
    ...
    class Light
    {
        ...
        public Light(P3D pos_, double intensity_)
        {
            pos = pos_;
            intensity = intensity_;
        }

        public double IntensityDiffusion(P3D norm, double k)
        {
            var scalar = norm.x * pos.x + norm.y * pos.y + norm.z * pos.z;
            var prod = Math.Sqrt(norm.x * norm.x + norm.y * norm.y + norm.z * norm.z) * Math.Sqrt(pos.x * pos.x + pos.y * pos.y + pos.z * pos.z);

            double cos = scalar / prod;

            return intensity * k * cos;
        }
    }
}
